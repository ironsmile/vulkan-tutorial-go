...
import (
	...
	"github.com/ironsmile/vulkan-tutorial-go/code/19_vertex_buffer/shaders"
...
const (
	title             = "Vulkan Tutorial: Vertex buffer creation"
	maxFramesInFlight = 2
)
...
func (a *VulkanTutorialApp) createVertexBuffer() error {
	bufferInfo := vk.BufferCreateInfo{
		...
		Size:        vk.DeviceSize(uint32(len(a.vertices)) * GetVertexSize()),
		Usage:       vk.BufferUsageFlags(vk.BufferUsageVertexBufferBit),
		SharingMode: vk.SharingModeExclusive,
	}

	var vertexBuffer vk.Buffer
	res := vk.CreateBuffer(a.device, &bufferInfo, nil, &vertexBuffer)
	if res != vk.Success {
	...
	vk.GetBufferMemoryRequirements(a.device, a.vertexBuffer, &memRequirements)
	memRequirements.Deref()

	memTypeIndex, err := a.findMemoryType(
	...
	res = vk.AllocateMemory(a.device, &allocInfo, nil, &vertexBufferMemory)
	if res != vk.Success {
		return fmt.Errorf("failed to allocate vertex buffer memory: %s", vk.Error(res))
	}
	a.vertexBufferMemory = vertexBufferMemory

	res = vk.BindBufferMemory(a.device, a.vertexBuffer, a.vertexBufferMemory, 0)
	if res != vk.Success {
		return fmt.Errorf("failed to bind buffer memory: %w", vk.Error(res))
	}

	var pData unsafe.Pointer
	vk.MapMemory(a.device, a.vertexBufferMemory, 0, bufferInfo.Size, 0, &pData)

	bytesSlice := unsafer.SliceToBytes(a.vertices)

	vk.Memcopy(pData, bytesSlice)
	vk.UnmapMemory(a.device, a.vertexBufferMemory)
...
func (a *VulkanTutorialApp) recordCommandBuffer(
	...
	vk.CmdDraw(commandBuffer, uint32(len(a.vertices)), 1, 0, 0)
