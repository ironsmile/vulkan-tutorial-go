...
import (
	...
	"github.com/ironsmile/vulkan-tutorial-go/code/25_sampler/shaders"
...
const (
	title             = "Vulkan Tutorial: Image view and sampler"
	maxFramesInFlight = 2
)
...
func (a *VulkanTutorialApp) createImageViews() error {
	for i, swapChainImage := range a.swapChainImages {
		...
		imageView, err := a.createImageView(swapChainImage, a.swapChainImageFormat)
		if err != nil {
...
func (a *VulkanTutorialApp) createIndexBuffer() error {
	bufferSize := vk.DeviceSize(uint32(len(a.indices)) * uint32(unsafe.Sizeof(a.indices[0])))
...
func (a *VulkanTutorialApp) createDescriptorSets() error {
	...
	for i := 0; i < maxFramesInFlight; i++ {
		...
		vk.UpdateDescriptorSets(a.device, 1, writes, 0, nil)
	}
...
func (a *VulkanTutorialApp) createTextureImageView() error {
	textureImageView, err := a.createImageView(a.textureImage, vk.FormatR8g8b8a8Srgb)
	if err != nil {
		return err
	}
